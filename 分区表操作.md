...
CREATE TABLE region_p4 PARTITION OF region  DEFAULT;
...
SELECT 
    ...
    parent.relname AS parent_table, 
    partition.relname AS partition_table, 
    pg_get_expr(partition.relpartbound, partition.oid) AS partition_range
FROM 
    pg_inherits AS inh
JOIN 
    pg_class AS parent ON inh.inhparent = parent.oid
JOIN 
    pg_namespace AS parent_ns ON parent.relnamespace = parent_ns.oid
JOIN 
    pg_class AS partition ON inh.inhrelid = partition.oid
JOIN 
    pg_namespace AS partition_ns ON partition.relnamespace = partition_ns.oid
WHERE 
    parent_ns.nspname = '__test_data__'
    AND parent.relname = 'template_range_hash';
...
WITH RECURSIVE partition_hierarchy AS (
    ...
    SELECT 
        ...
        partition.relname AS partition_table,
        pg_get_expr(partition.relpartbound, partition.oid) AS partition_range,
        partition.oid AS partition_oid
    FROM 
        pg_inherits AS inh
    JOIN 
        pg_class AS parent ON inh.inhparent = parent.oid
    ...
    -- 递归查找下一级分区
    SELECT 
        ph.partition_schema AS parent_schema,
        ph.partition_table AS parent_table,
        partition_ns.nspname AS partition_schema,
        partition.relname AS partition_table,
        pg_get_expr(partition.relpartbound, partition.oid) AS partition_range,
        partition.oid AS partition_oid
    FROM 
        partition_hierarchy AS ph
    JOIN 
        pg_inherits AS inh ON ph.partition_oid = inh.inhparent
    JOIN 
        pg_class AS partition ON inh.inhrelid = partition.oid
    JOIN 
        pg_namespace AS partition_ns ON partition.relnamespace = partition_ns.oid
...
WITH RECURSIVE partition_info AS (
    ...
    SELECT
        ...
        NULL::text AS subpartition_name, -- 初始时没有子分区
        pg_get_expr(partition.relpartbound, partition.oid) AS partition_range, -- 获取分区范围
        partition.oid AS partition_oid
    FROM
        pg_inherits AS inh
    JOIN
    ...
    JOIN
        pg_namespace AS partition_ns ON partition.relnamespace = partition_ns.oid
    WHERE
        parent_ns.nspname = '__test_data__' -- 你需要的 schema
        AND parent.relname = 'template_range_hash' -- 你需要的表名
    ...
    SELECT
        ...
        pi.table_name,
        pi.partition_name,
        subpartition.relname AS subpartition_name,
        pg_get_expr(subpartition.relpartbound, subpartition.oid) AS partition_range, -- 获取子分区的范围
        subpartition.oid AS partition_oid
    FROM
        partition_info AS pi
    JOIN
        pg_inherits AS inh ON pi.partition_oid = inh.inhparent
    JOIN
        pg_class AS subpartition ON inh.inhrelid = subpartition.oid
    JOIN
        pg_namespace AS subpartition_ns ON subpartition.relnamespace = subpartition_ns.oid
)
SELECT
    table_schema,
    table_name,
    partition_name,
    subpartition_name,
    partition_range -- 显示分区范围
FROM
    partition_info
WHERE
    subpartition_name IS NOT NULL; -- 过滤掉 NULL 值
